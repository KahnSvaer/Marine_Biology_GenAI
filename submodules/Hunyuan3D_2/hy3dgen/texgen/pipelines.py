...
def _delight_image_prompt_worker(config, images_prompt, save_path):
...
class Hunyuan3DPaintPipeline:
    def run_delight_in_process(self, images_prompt, save_path="memory_results.pkl"):
        p = mp.Process(target=_delight_image_prompt_worker, args=(self.config, images_prompt, save_path))
        p.start()
        p.join()
    ...
    def load_Multiview_Diffusion_Net(self):
        self.models['multiview_model'] = Multiview_Diffusion_Net(self.config)
        if self.to_enable_model_cpu_offload:
            self.models['multiview_model'].pipeline.enable_model_cpu_offload(
                gpu_id=0, device=self.config.device)
        else:
            self.models['multiview_model'].pipeline.to(self.config.device)
    ...
    def enable_model_cpu_offload(self, gpu_id: Optional[int] = None, device: Union[torch.device, str] = "cuda"):
        ...
        # self.models['super_model'].pipeline.enable_model_cpu_offload(gpu_id=gpu_id, device=device)
    ...
    def __call__(self, mesh, image):
        ...
        self.load_Multiview_Diffusion_Net()
