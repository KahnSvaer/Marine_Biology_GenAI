...
class Light_Shadow_Remover():
    def __init__(self, config):
        print("Loading Light and Shadow Remover..., Path: ", config.light_remover_ckpt_path)
    ...
    def unload(self):
        print("[Light_Shadow_Remover] Unloading and freeing memory...")
        if hasattr(self, "pipeline"):
            try:
                self.pipeline.to("cpu")  # Moves to CPU first to free GPU
            except:
                pass
            del self.pipeline
            self.pipeline = None
